...
> 当前版本：`0.1.2-beta.1`。**请先备份存档**，并只在测试存档中使用。  
...
- 普通区块继续使用原版 `Y=-64..319` 和连续子区块数组。
- 扩展子区块使用 `int sectionY`（子区块 Y 坐标）稀疏存储；中间没有方块的高度不占用子区块。
- 可选稀疏子区块运算规则，减少大量卡顿。
- 最后一个非空气方块被移除后，空子区块自动回收。
- 扩展数据随区块 NBT 保存，退出并重新进入世界后仍然存在。
- 每个区块或区块群可配置独立的垂直建筑区域，区域不能重叠。
- 原版高度范围不可修改，所有扩展范围按完整 16 格子区块向外对齐。
...
2. 输入最低与最高 Y；输入框失焦后会自动向外对齐到完整子区块。
...
- `钠`（Sodium `0.8.12/0.8.13`）与 `铷`（Embeddium `1.0.x`）兼容已实现；光影已验证可用。
- Sable 物理兼容已实现：Create Aeronautics 的物理化结构可在扩展高度与方块碰撞。
- 扩展高度暂不支持方块实体。
- 光照更新已实现
- scheduled tick、随机刻、流体刻、红石、生物生成和天气等分段模拟规则尚未实现。
- 实验性 double 坐标模式尚未实现。
- Create、Valkyrien Skies 的兼容不属于当前 Beta 的保证范围；Create Aeronautics/Offroad 的物理碰撞兼容（基于 Sable）已提供，但机械动力方块实体等功能暂不支持。
